...
int main(int argc, char *argv[]) {
    ...
    vector<float> y4; /*Normalized Second Moment*/
    vector<float> y6; /*Normalized Second Moment*/
    vector<float> y8; /*Normalized Second Moment*/
    vector<float> y10; /*Normalized Second Moment*/
        ...
        for (long t = 0; t < p; ++t) {
            ...
            for (int i = 1; i <= reps.size(); i++) { 
                ...
                if (isFourthPower(lfourthmaybe, p)) { 
                    ...
                    value2 += pow(data[p * (i-1) + B], 2);
                    value4 += pow(data[p * (i-1) + B], 4);
                    value6 += pow(data[p * (i-1) + B], 6);
                    value8 += pow(data[p * (i-1) + B], 8);
                    value10+= pow(data[p * (i-1) + B],10);
                    break;
                }
            }
    ...
    for (size_t i = 0; i < x.size(); ++i) {
        ...
        output_file << x[i] << "," << y[i] << "," << y4[i] << "," << y6[i] << "," << y8[i] << "," << y10[i] << std::endl; 
